...
class Exercise4Renderer : public OpenGLRenderEntry {
	...
	void OpenGLRenderEntry::PreRenderFunc() {
		...
		SetFragmentPath("Exercise4FShader.frag");
		...
		this->shaderClass->setFloat("mixRatio", mixRatio);
	}
	...
	void RenderKeyUpCallback() override {
		this->mixRatio += 0.01f;
	}

	void RenderKeyDownCallback() override {
		this->mixRatio -= 0.01f;
	}
